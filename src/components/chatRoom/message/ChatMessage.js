import { Message } from "../../styles/Div";
import { auth } from "../../../firebase/init";

function ChatMessage(info) {
  const { currentUserUID } = auth.currentUser;

  const { photoURL, content, type, uid } = info.message;
  console.log(currentUserUID === uid); // TODO - CURRENT USER MESSAGES SHOULD BE PLACED ON RIGHT
  ...
  return (
    <>
      <Message>
        ...
        <div> {type === "text" ? <p>{content}</p> : <img src={content} />}</div>
      </Message>
    </>
  );
}
